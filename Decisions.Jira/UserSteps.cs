	...
	public static class UserSteps
	{
		public static JiraResult CreateUser(JiraCredentials Credentials, JiraUserModel JiraUserModel)
		{
			try
			{
				...
				var response = new JiraUtility().GetClient(Credentials).PostAsync("user", content).Result;
		...
		public static JiraResult DeleteUser(JiraCredentials Credentials, string AccountId)
		{
			try
			{
				var response = new JiraUtility().GetClient(Credentials).DeleteAsync($"user?accountId={AccountId}").Result;
				var responseString = response.Content.ReadAsStringAsync().Result;
